...
2. 存储密度对系统的可用性和可靠性都会有一定的伤害。
    ```sh
    # 存储密度由两部分组成：
    # A. 单机可支持的硬盘数：硬盘数越少，需要的服务器台数越多，宕机概率越高，可靠性越低
    # B. 硬盘的容量：       容量越大，恢复时间越长，可用性越低
    ```
...
1. **缓存诞生的背景**
    ```sh
    # 因为如果存储本身非常匹配业务场景的话，它不应该需要缓存在它前面挡一道，内部自己就有缓存。
    # 实现一个存储很难，所以存储的业务场景匹配性很难做到处处都很好。
    # 所以把一个复杂的 F(x) 缓存起来，更根本的原因还是存储和业务场景不那么直接匹配所致
    #
    # 【缓存是存储系统为了更好设配业务逻辑读功能而诞生的补丁。memcached不太完美,一致性缺陷】
    #   对于一个确定的 x 值，如果 F(x) 永远不变，这就没问题。但如果 F(x) 值会发生变化，会有多个版本的值，
    #   那就有可能会出现并发的两个 F(x) 请求得到的结果不同，从而导致缓存中的值和存储中的值不一致。
    # 
    # 【写】出现事务（Transaction），是为了改善存储的业务场景“写操作”的匹配性,把一个复杂操作包装成一个原子操作。
    # 【读】出现缓存（Cache），则是为了改善存储的业务场景“读操作”的匹配性，提升高频读操作的效率。
    ```

2. **合格的存储要抗住什么问题？1.持久性：多机房多副本；2.重试的友好性**

- **几种常见的缓存**
1. **memcached： 存在的一致性缺陷**

2. **groupcached**
    ```sh
    # 【值不可修改】
    #  groupcached 没有memcached 中的缓存失效时间这样的概念。因为值是不会过时的，它只会因为内存不足而被淘汰。一致性问题也被解决了。
    #  groupcached 对使用者来说是有挑战的，某种意义上来说，它鼓励我们用函数式编程的方式来实现业务逻辑。
    #
    # 【值不可修改的一种实现】
    #  以键值存储为例，它存储的是 key => (value, ver)，而 groupcache 存储的是 (key, ver) => value。
    #  这样就可以做到存储的值改变而 groupcache 值不变。
    ```

3. **Redis**
    ```sh
    # 【定期持久化】： Redis 基于内存的存储，支持定期写到外存中。
    # 【缓存】：当内存使用达到上限后，Redis 就会执行缓存淘汰算法。
    # 【key => document】：它的值可以是各类数据结构，比如：字符串，哈希表，列表，集合，有序集合（支持 Range 查询），等等。
    # 【外部接口】：Redis 支持执行 Lua 脚本来做存储过程。
    # 
    # 【存储系统】：
    #   Redis 如果要作为存储的话，必须保证用多机房多副本的方式，才能保证在持久性这一点上能够达标。
    #   但是多机房多副本这样的方式，显然实施条件过于苛刻。会有多少企业仅仅是为了部署 Redis 去搞多个机房呢？
    #
    #   在 Redis 的协议中，有不少请求用户很友好，但是对重试并不友好。比如，LPUSH 请求用来给列表（List）增加一个元素。
    #   但是在重试时一个不小心，我们很可能就往列表中添加了多个相同的元素进去。
    ```
