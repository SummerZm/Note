...
- 3. 从项目中胖model的职责
    ```sh
    # 务逻辑，对外暴露业务接口。它也是 Model 的最本职的工作。
...
- **胖后端**
    1. 有些公司会基于 “胖后端” 模式。这意味着大部分的前端用户行为，都是由后端支持的。
    2. 胖后端模式的好处是 Web 代码比较安全。因为别人看不到完整的 Web 业务逻辑代码。
    3. 胖后端模式的缺点是没办法支持离线。大部分的用户交互都需要 Web 后端来响应，一旦断了网就什么都干不了了。

- **前后端通信协议的维度**
    1. 网络协议通常包含两个层面的意思： A. 网络协议的载体(http/tcp/udp)  B. 网络协议承载的具体业务逻辑
    2. 架构同也有两个层面，只是它们在不同的维度
        ```sh
        # a. 基础架构的维度: 关心网络协议的协议栈选择什么，是基于 HTTP 还是基于自定义的二进制协议
        # b. 应用架构的维度: 关心网络协议的业务逻辑，判断它是否自然体现业务需求
        ```
...
- 如何看待json在代码中满天飞的问题？
- 支持多实例听起来是一项简单的工作，但是对很多工程师来说实际上并不简单。不少初级工程师写代码往往容易全局变量满天飞，模块之间相互传递信息不假思索地基于全局变量来完成。这些不良习惯会导致代码极难控件化。
